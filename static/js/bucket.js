;(function() {
    ...
    function set_jwt_to_storage(jwt){
        localStorage.setItem('auth-jwt', jwt);
    }
    function remove_jwt_from_storage(){
        localStorage.removeItem('auth-jwt');
    }

    function login(success_do) {
        Swal({
            title: '请登录',
            html:
              '<label>用户名</label>'+
            ...
            if (formValues.value) {
                let username = formValues.value[0];
                let password = formValues.value[1];
                $.ajax({
                    url: build_url_with_domain_name('/api/v1/jwt-token/'),
                    type: 'post',
                    ...
                    if (result.code === 200){
                        set_jwt_to_storage(result.token);
                        if(typeof success_do === "function")//是函数
                            success_do();
                    }else{
                ...
                }).fail(function(result,err,errmsg) {
                    if (result.status===401){
                        show_warning_dialog("登录认证失败");
                    }else{
                        show_warning_dialog(result.statusText);
                    }                   
                });
            }
        });            
    ...
    window.onload = function() {
        (async function() {
            let jwt = get_jwt_from_storage();
            if (!jwt){
                show_warning_dialog("请先登录");
                login(get_buckets_and_render);
            }else{
