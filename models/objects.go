...
type Bucket struct {
	...
	CollectionName   string      `gorm:"type:varchar(50)" json:"-"` //存储桶对应的表名
	AccessPermission uint8       `gorm:"" json:"access_permission"` //访问权限
	SoftDelete       bool        `gorm:"" json:"-"`                 // True->删除状态
	ModifiedTime     time.Time   `gorm:"" json:"-"`                 // 修改时间可以指示删除时间
	ObjsCount        uint32      `gorm:"" json:"-"`                 //桶内对象的数量
	Size             uint64      `gorm:"" json:"-"`                 //桶内对象的总大小
	StatsTime        time.Time   `gorm:"" json:"-"`                 //统计时间
}
