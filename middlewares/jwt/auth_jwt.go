// Package jwt modyfied base on package github.com/appleboy/gin-jwt
package jwt
...
type GinJWTMiddleware struct {
	...
	// Do nothing, don't response, if it is not JWT header
	DoNothingIfNotJWTHeader bool

	GetJWTFromBodyFunc func(c *gin.Context) (string, error)
}
...
func (mw *GinJWTMiddleware) MiddlewareInit() error {
	...
	if mw.GetJWTFromBodyFunc == nil {
		mw.GetJWTFromBodyFunc = func(c *gin.Context) (string, error) {
			tokenForm := struct {
				Token string `json:"token" form:"token"`
			}{}
			if err := c.ShouldBind(&tokenForm); err != nil {
				return "", ErrEmptyBodyToken
			}
...
func (mw *GinJWTMiddleware) CheckIfTokenExpire(c *gin.Context) (jwt.MapClaims, error) {
	// token, err := mw.ParseToken(c)
	tokenString, err := mw.GetJWTFromBodyFunc(c)
	if err != nil {
		return nil, err
	}
